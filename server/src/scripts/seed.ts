import { getSession, closeDriver } from '../config/neo4j';
...
const seedData = async () => {
  ...
  try {
    ...
    const projects: { id: string; name: string; description: string; stars: number; status: string }[] = [];
    for (let i = 1; i <= 200; i++) {
    ...
    const jobs: { id: string; title: string; description: string; salaryRange: string; location: string; type: string; companyId: string }[] = [];
    for (let i = 1; i <= 100; i++) {
    ...
    const devSkills: { devId: string; skillId: string; level: string }[] = [];
    ...
    const devTech: { devId: string; techId: string; yearsOfExperience: number }[] = [];
    ...
    const companyTech: { companyId: string; techId: string }[] = [];
    companies.forEach((c) => {
    ...
    const jobSkills: { jobId: string; skillId: string }[] = [];
    jobs.forEach((j) => {
    ...
    const jobTech: { jobId: string; techId: string }[] = [];
    jobs.forEach((j) => {
    ...
    const projectTech: { projectId: string; techId: string }[] = [];
    const repoTech: { repoId: string; techId: string }[] = [];
    ...
    const workedAt: { devId: string; companyId: string; role: string; startDate: string; endDate: string | null }[] = [];
    developers.forEach((d) => {
    ...
    const contributions: { devId: string; repoId: string; commitsCount: number }[] = [];
    developers.forEach((d) => {
    ...
    const memberOf: { devId: string; communityId: string }[] = [];
    developers.forEach((d) => {
    ...
    const knows: { devId1: string; devId2: string; since: string }[] = [];
    developers.forEach((d, idx) => {
    ...
    const uniqueKnowsMap = new Map<string, typeof knows[0]>();
    knows.forEach((k) => {
    ...
    const mentorships: { menteeId: string; mentorId: string }[] = [];
    developers.forEach((d) => {
